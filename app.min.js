    ...
    if (current && current.images.length > 1) {
      ...
      mobileZoomImg._swipeTouchStart = (e) => {
        if (e.touches.length === 1) {
          ...
          // Если в зуме - принудительно сбрасываем флаг свайпа
          if (isZoomed) {
            isSwiping = false;
          }
        }
      ...
      mobileZoomImg._swipeTouchMove = (e) => {
        if (e.touches.length === 1) {
          ...
          // Если в зуме - ПОЛНОСТЬЮ блокируем все свайпы для листания
          if (isCurrentlyZoomed) {
            isSwiping = false; // Принудительно сбрасываем флаг свайпа
            // Блокируем все горизонтальные движения, которые могут вызвать листание
            if (deltaX > 3) {
              e.preventDefault();
              e.stopPropagation();
            }
