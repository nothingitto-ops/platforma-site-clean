...
function preloadImages() {
  ...
  console.log(`Preloading ${imagesToPreload.length} images for smooth transitions`);
}
  ...
  try {
    ...
    setTimeout(() => {
      ...
      ctaButtons.forEach(btn => {
        ...
        console.log('Fixed CTA button styles');
      });
    }, 100);
...
function loadHero() {
  ...
  if (hero) {
    ...
    // Плавно скрываем баннер перед сменой
    ...
    setTimeout(() => {
      ...
      // Плавно показываем новый баннер
      requestAnimationFrame(() => {
        hero.style.visibility = 'visible';
        hero.style.opacity = '1';
      });
      
      console.log('Hero banner loaded:', banner);
    }, 100);
  } else {
...
function renderCatalog(filterType = 'all') {
  ...
  // Плавно скрываем каталог перед обновлением
  catalog.style.opacity = '0';
  catalog.style.visibility = 'hidden';
  
  // Небольшая задержка для плавного перехода
  setTimeout(() => {
    // Фильтруем товары по текущей вкладке
    let filteredProducts = products.filter(p => p.section === currentTab);
    ...
    // Рендерим карточки
    catalog.innerHTML = filteredProducts.map(item => createCard(item)).join('');
    
    // Добавляем обработчики событий
    addCardEventListeners();
    
    // Плавно показываем каталог
...
function openMobileModal(item, section) {
  ...
  // Добавляем обработчики кликов для листания фото в мобильной версии
  ...
  if (mobileMainImage) {
    ...
    mobileMainImage.addEventListener('click', handleMobileViewerClick);
  }
...
function closeMobileZoom() {
  ...
  if (mobileZoomModal) {
    ...
    document.body.style.overflow = 'hidden'; // Возвращаем к мобильной модалке
  }
}
  ...
  if (mobileZoomImg) {
    ...
    mobileZoomImg.addEventListener('touchstart', (e) => {
      if (e.touches.length === 1) {
        ...
        // Два пальца - это зум, не обрабатываем свайпы
        isZooming = true;
      }
    }, { passive: true });
    ...
    mobileZoomImg.addEventListener('touchend', (e) => {
      if (isZooming) return; // Пропускаем если был зум
...
function setupDesktopModalHandlers() {
  console.log('setupDesktopModalHandlers called');
  ...
  if (modalLeft) {
    ...
    console.log('Added click handler to modalLeft');
  }
  ...
  if (zoomBtn) {
    ...
    zoomBtn.addEventListener('mousedown', (e) => {
      console.log('zoomBtn mousedown:', e);
    });
    
    zoomBtn.addEventListener('mouseup', (e) => {
      console.log('zoomBtn mouseup:', e);
    });
    
    console.log('Added click handler to zoomBtn');
  } else {
    console.log('zoomBtn not found!');
  }
}
...
function handleDesktopZoom() {
  console.log('handleDesktopZoom called');
  if (!current) {
  ...
  if (mainImg && modalLeft) {
    ...
    console.log('Failed to find image elements');
  }
}
...
function handleEscKey(e) {
  if (e.key === 'Escape') {
    ...
    if (modalLeft && modalLeft.classList.contains('zoomed')) {
      ...
      console.log('Zoom disabled via ESC');
    }
  }
}
