...
// Оптимизированная предзагрузка изображений - только первые изображения продуктов
function preloadImages() {
  // Предзагружаем только баннеры и первые изображения продуктов
  const imagesToPreload = ['img/bannerh.jpg', 'img/banner.jpg'];
  
  // Добавляем только первые изображения продуктов для быстрой загрузки
  products.forEach(product => {
    if (product.images.length > 0) {
      imagesToPreload.push(`img/${product.images[0]}`);
    }
  });
...
function createCard(item) {
  ...
  return `
    <div class="card" data-id="${item.id}">
      <div class="card-img">
        <img src="img/${item.images[0]}" alt="${item.title}" loading="eager">
      </div>
      <h3>${item.title}</h3>
...
function setMainImage(src) {
  ...
  if (viewerImg) {
    // Быстрая загрузка без предзагрузки
...
function openMobileModal(item, section) {
  ...
  if (item.images && item.images.length > 0) {
    // Предзагружаем только первое изображение для скорости
    const img = new Image();
    img.loading = 'eager';
    img.fetchPriority = 'high';
    img.src = `img/${item.images[0]}`;
  }
  ...
  // Быстрое открытие без лишних проверок
  ...
  if (mobileCompositionBtn) {
    ...
    // Создаем новый обработчик только для клика с правильным замыканием
    mobileCompositionBtn._clickHandler = function(e) {
      e.preventDefault();
      e.stopPropagation();
      e.stopImmediatePropagation();
      
      // Используем current вместо item для актуальных данных
    ...
    if (current && current.images.length > 1) {
      ...
      mobileMainImage._touchEndHandler = (e) => {
        if (e.changedTouches.length === 1 && isSwiping) {
          ...
          // Оптимизированные условия - меньше вычислений
          if (Math.abs(deltaX) > 50 && deltaTime < 600) {
            ...
            // Быстрое обновление изображения с предзагрузкой
            const mobileMainImg = document.getElementById('mobileMainImg');
            if (mobileMainImg && current.images[curIndex]) {
              // Предзагружаем следующее изображение для плавности
              const nextIndex = (curIndex + 1) % current.images.length;
              const nextImg = new Image();
              nextImg.src = `img/${current.images[nextIndex]}`;
  ...
  // Добавляем обработчики событий сразу после открытия модалки
...
function addMobileEventHandlers() {
  ...
  // Убираем обработчик кнопки зума - больше не нужен
...
function removeMobileEventHandlers() {
  ...
  if (mobileZoomBtn) {
    // Убираем ссылку на несуществующую функцию toggleMobileZoom
    mobileZoomBtn.onclick = null;
  }
...
// Убираем функцию зума - больше не нужна

// Убираем все функции панорамирования - больше не нужны
