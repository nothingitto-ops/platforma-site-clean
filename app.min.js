    ...
    if (current && current.images.length > 1) {
      ...
      mobileZoomImg._swipeTouchStart = (e) => {
        if (e.touches.length === 1) {
          ...
          const matrix = new DOMMatrix(transform);
          const currentScale = matrix.a;
          isZoomed = currentScale > 1.05; // Уменьшаем порог для более стабильного определения зума
      ...
      mobileZoomImg._swipeTouchMove = (e) => {
        if (e.touches.length === 1) {
          ...
          const matrix = new DOMMatrix(transform);
          const currentScale = matrix.a;
          const isCurrentlyZoomed = currentScale > 1.05; // Уменьшаем порог для более стабильного определения зума
      ...
      mobileZoomImg._gestureEnd = (e) => {
        ...
        // Если масштаб больше 1.05 - обновляем флаг зума
        if (currentScale > 1.05) {
          isZoomed = true;
        }
