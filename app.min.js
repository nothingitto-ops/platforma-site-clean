...
function openMobileModal(item, section) {
    ...
    mobileMainImage.addEventListener('touchend', (e) => {
      if (e.changedTouches.length === 1) {
        ...
        if (Math.abs(deltaX) > 50 && deltaTime < 300 && deltaY < 30 && current && current.images.length > 1) {
          // Упрощенная защита от множественных срабатываний
  ...
  if (item.images.length > 1) {
    item.images.forEach((imagePath, idx) => {
      ...
      // Принудительно устанавливаем стили для активной точки
      if (idx === curIndex) {
...
function updateMobileDots() {
  ...
  // console.log(`updateMobileDots: curIndex = ${curIndex}, active dots = ${Array.from(dots).filter(d => d.classList.contains('active')).length}`);
}
...
function openMobileZoom() {
  ...
  if (zoomDotsContainer && current && current.images.length > 1) {
    ...
    current.images.forEach((imagePath, idx) => {
      ...
      // Принудительно устанавливаем стили при создании
      if (idx === curIndex) {
        dot.style.background = '#000';
        dot.style.backgroundColor = '#000';
      } else {
      ...
      // Убираем клики по точкам в зум-версии - только свайпы для листания
      // dot.addEventListener('click', (e) => {
      //   e.preventDefault();
      //   e.stopPropagation();
      //   
...
function updateZoomDots() {
  ...
  dots.forEach((dot) => {
    ...
    // Принудительно устанавливаем стили для зум-точек
    if (isActive) {
      dot.style.background = '#000';
      dot.style.backgroundColor = '#000';
    } else {
  ...
  // Отладочная информация (убрана для чистоты консоли)
  // console.log(`updateZoomDots: curIndex = ${curIndex}, active dots = ${Array.from(dots).filter(d => d.classList.contains('active')).length}`);
}
...
function addMobileZoomEventHandlers() {
  ...
  if (mobileZoomImg) {
    ...
    let isZooming = false;
    let initialScale = 1;
    let isProcessingZoomSwipe = false; // Защита от множественных срабатываний в зуме
    ...
    mobileZoomImg.addEventListener('touchstart', (e) => {
      if (e.touches.length === 1) {
        ...
        touchStartTime = Date.now();
        isZooming = false;
        isProcessingZoomSwipe = false; // Сбрасываем флаг при новом касании
        ...
        if (transform !== 'none') {
          initialScale = parseFloat(transform.split(',')[3]) || 1;
        }
    ...
    mobileZoomImg.addEventListener('touchmove', (e) => {
      ...
      // Если один палец и не зум - разрешаем все движения для зума
    ...
    mobileZoomImg.addEventListener('touchend', (e) => {
      ...
      const transform = window.getComputedStyle(mobileZoomImg).transform;
      const currentScale = transform !== 'none' ? parseFloat(transform.split(',')[3]) || 1 : 1;
      
      // Если зум-аут (масштаб близок к 1), возвращаем к исходному состоянию
      if (isZooming && currentScale <= 1.1) {
      ...
      if (e.changedTouches.length === 1 && current && current.images.length > 1) {
        ...
        if (Math.abs(deltaX) > Math.abs(deltaY) && Math.abs(deltaX) > 50 && deltaTime < 300 && deltaY < 30) {
          // Упрощенная защита от множественных срабатываний
          if (deltaTime < 200) return;
