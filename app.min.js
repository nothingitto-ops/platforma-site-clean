...
function addFullscreenEventHandlers() {
  ...
  if (fullscreenImg) {
    // Добавляем обработчики свайпов для листания фотографий
    let touchStartX = 0;
    let touchStartY = 0;
    let touchStartTime = 0;
    let isSwiping = false;
    ...
    fullscreenImg._fullscreenTouchStart = (e) => {
      if (e.touches.length === 1) {
        ...
        // Игнорируем жесты, которые происходят слишком быстро после предыдущего
    ...
    fullscreenImg._fullscreenTouchMove = (e) => {
      if (e.touches.length === 1) {
        const currentX = e.touches[0].clientX;
        const currentY = e.touches[0].clientY;
        const deltaX = Math.abs(currentX - touchStartX);
        ...
        // Более строгое определение свайпа
    ...
    fullscreenImg._fullscreenTouchEnd = (e) => {
      if (e.changedTouches.length === 1) {
        ...
        // Более надежное определение свайпа
        if (isSwiping && deltaX > 60 && deltaX > deltaY * 1.5 && timeDiff < 500) {
          const swipeDeltaX = touchStartX - touchEndX;
          ...
          // Сбрасываем состояние свайпа после выполнения
          isSwiping = false;
          lastGestureTime = Date.now();
        } else if (deltaX < 15 && deltaY < 15 && timeDiff < 300) {
          // Если это был тап (маленькое движение и короткое время) - закрываем полноразмерный просмотр
          closeFullscreenGallery();
          
          // Сбрасываем состояние свайпа после тапа
          isSwiping = false;
          lastGestureTime = Date.now();
        } else {
          // Сбрасываем состояние свайпа для всех остальных случаев
          isSwiping = false;
          lastGestureTime = Date.now();
        }
      }
    ...
    fullscreenImg.addEventListener('touchmove', fullscreenImg._fullscreenTouchMove, { passive: true });
...
function removeFullscreenEventHandlers() {
  ...
  if (fullscreenImg) {
    ...
    fullscreenImg.removeEventListener('touchmove', fullscreenImg._fullscreenTouchMove);
