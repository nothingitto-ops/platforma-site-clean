...
let lastSwipeTime = 0; // Защита от множественных свайпов
...
async function loadProducts() {
  try {
    ...
    // Сортируем товары по полю order для правильного отображения
...
function applyFilter(filterType) {
  ...
  // Фильтрация с пересозданием DOM
  filterCatalog(filterType);
}

// Функция фильтрации с пересозданием DOM
function filterCatalog(filterType = 'all') {
  ...
  // Создаем карточки только для отфильтрованных товаров
  const filteredCards = filteredProducts.map((item, index) => createCard(item, index)).join('');
  catalog.innerHTML = filteredCards;
...
function openMobileModal(item, section) {
  current = item;
  curIndex = 0;
  lastSwipeTime = 0; // Сбрасываем защиту от множественных свайпов
  ...
  if (mobileCompositionBtn) {
    ...
    mobileCompositionBtn._clickHandler = function(e) {
      ...
      // Используем текущий товар из глобальной переменной current
      ...
      if (this.classList.contains('active')) {
        ...
        mobileDesc.textContent = currentItem.desc;
      } else {
        // Показываем состав
        this.classList.add('active');
        this.textContent = '?';

        mobileDesc.textContent = currentItem.meta;
      }
    };
      ...
      mobileMainImage._touchEndHandler = (e) => {
        if (e.changedTouches.length === 1 && isSwiping) {
          ...
          if (Math.abs(deltaX) > 5) {
            ...
            // Защита от множественных свайпов - минимум 300ms между свайпами
            if (currentTime - lastSwipeTime < 300) {
              console.log('Swipe blocked - too fast');
              return;
            }
            ...
            console.log('Mobile swipe: oldIndex =', oldIndex, 'newIndex =', curIndex, 'deltaX =', deltaX);
            
            // Мгновенное обновление изображения без задержек
            const mobileMainImg = document.getElementById('mobileMainImg');
            if (mobileMainImg) {
              // Принудительно обновляем все счетчики сразу
              updateAllCounters();
...
function updateMobileImageCounter() {
  ...
  if (mobileDots && current && current.images.length > 1) {
    ...
    const counterText = `${curIndex + 1} / ${current.images.length}`;
    mobileDots.textContent = counterText;
    console.log('updateMobileImageCounter:', counterText);
  }
}

// Универсальная функция для обновления всех счетчиков
function updateAllCounters() {
  console.log('updateAllCounters: curIndex =', curIndex, 'images.length =', current ? current.images.length : 0);
...
function addMobileEventHandlers() {
  ...
  // Добавляем обработчик клика по изображению для полноразмерного просмотра
  if (mobileMainImage) {
    mobileMainImage.removeEventListener('click', openFullscreenGallery);
    ...
    mobileMainImage.addEventListener('touchmove', (e) => {
      if (e.touches.length === 1) {
        ...
        // Если движение больше 10px - это не клик
    ...
    mobileMainImage.addEventListener('click', (e) => {
      ...
      // Проверяем, что это действительно клик, а не свайп
...
function updateZoomDots() {
  ...
  if (zoomDotsContainer && current && current.images.length > 1) {
    // Используем textContent вместо innerHTML для скорости
    const counterText = `${curIndex + 1} / ${current.images.length}`;
    zoomDotsContainer.textContent = counterText;
    console.log('updateZoomDots:', counterText);
  }
}
    ...
    if (current && current.images.length > 1) {
      ...
      mobileZoomImg._swipeTouchMove = (e) => {
        if (e.touches.length === 1) {
          ...
          // Если в зуме - не обрабатываем свайпы
          if (isZoomedNow) {
            isSwiping = false;
            return;
          }
      ...
      mobileZoomImg._clickHandler = (e) => {
        ...
        const matrix = new DOMMatrix(transform);
        const currentScale = matrix.a;
        const isCurrentlyZoomed = currentScale > 1.02;
      ...
      mobileZoomImg._swipeTouchEnd = (e) => {
        ...
        // Если в зуме - НЕ обрабатываем свайпы для листания
        if (isCurrentlyZoomed) {
          isSwiping = false;
          return;
        }
        ...
        if (e.changedTouches.length === 1 && isSwiping) {
          ...
          if (Math.abs(deltaX) > 50 && deltaTime < 500 && deltaY < 100) {
            const currentTime = Date.now();
            // Защита от множественных свайпов - минимум 500ms между свайпами в зум режиме
            if (currentTime - lastSwipeTime < 500) {
              console.log('Zoom swipe blocked - too fast');
              return;
            }
            lastSwipeTime = currentTime;
            ...
            console.log('Zoom swipe: oldIndex =', oldIndex, 'newIndex =', curIndex, 'deltaX =', deltaX);
            ...
            // Обновляем все счетчики изображений
            updateAllCounters();
          }
        }
      };
      ...
      mobileZoomImg.addEventListener('gesturestart', (e) => {
        // При начале жеста зума сбрасываем флаг свайпа
        isSwiping = false;
      }, { passive: true });
      
      mobileZoomImg.addEventListener('gesturechange', (e) => {
        // При изменении жеста зума сбрасываем флаг свайпа
        isSwiping = false;
      }, { passive: true });
...
function closeAllModals() {
  ...
  if (fullscreenGallery) {
    fullscreenGallery.classList.remove('open');
...
// ===== ФУНКЦИИ ДЛЯ ПОЛНОРАЗМЕРНОГО ПРОСМОТРА =====
function openFullscreenGallery(e) {
  ...
  if (!fullscreenGallery || !fullscreenImg || !mobileMainImg) {
    return;
  }
  
  lastSwipeTime = 0; // Сбрасываем защиту от множественных свайпов
  
  // Копируем текущее изображение в полноразмерный просмотр
  fullscreenImg.src = mobileMainImg.src;
  fullscreenImg.alt = '';
  
  // Показываем счетчик фотографий
  if (current && current.images.length > 1) {
  ...
  // Открываем полноразмерный просмотр
  fullscreenGallery.classList.add('open');
  document.body.style.overflow = 'hidden';
...
function closeFullscreenGallery() {
  ...
  if (fullscreenGallery) {
    fullscreenGallery.classList.remove('open');
    // Возвращаемся в модалку, не закрываем все
    document.body.style.overflow = 'hidden';
    
    // Синхронизируем изображение в модальном окне с текущим индексом
    updateMobileImage();
...
function addFullscreenEventHandlers() {
  const fullscreenGallery = document.getElementById('fullscreenGallery');
  const fullscreenImg = document.getElementById('fullscreenImg');
  const fullscreenCloseBtn = document.getElementById('fullscreenCloseBtn');
  
  if (fullscreenCloseBtn) {
    fullscreenCloseBtn.addEventListener('click', closeFullscreenGallery);
  }
  
  if (fullscreenImg) {
    // Простая и надежная логика свайпов
    let startX = 0;
    let startY = 0;
    
    fullscreenImg._fullscreenTouchStart = (e) => {
      if (e.touches.length === 1) {
    ...
    fullscreenImg._fullscreenTouchEnd = (e) => {
      if (e.changedTouches.length === 1) {
        const endX = e.changedTouches[0].clientX;
        const endY = e.changedTouches[0].clientY;
        const deltaX = startX - endX; // Исправляем формулу для синхронизации с другими режимами
        const deltaY = endY - startY;
        
        // Если движение больше горизонтальное чем вертикальное и достаточно длинное
        if (Math.abs(deltaX) > Math.abs(deltaY) && Math.abs(deltaX) > 50) {
          const currentTime = Date.now();
          // Защита от множественных свайпов - минимум 300ms между свайпами
          if (currentTime - lastSwipeTime < 300) {
            console.log('Fullscreen swipe blocked - too fast');
            return;
          }
          lastSwipeTime = currentTime;
    ...
    // Добавляем обработчик клика по изображению для закрытия
    fullscreenImg._fullscreenClick = (e) => {
      closeFullscreenGallery();
    };
    
    fullscreenImg.addEventListener('touchstart', fullscreenImg._fullscreenTouchStart, { passive: true });
    fullscreenImg.addEventListener('touchend', fullscreenImg._fullscreenTouchEnd, { passive: false });
    fullscreenImg.addEventListener('click', fullscreenImg._fullscreenClick);
  }
  
  // Добавляем обработчик клика по фону для закрытия
  if (fullscreenGallery) {
    fullscreenGallery._fullscreenBackgroundClick = (e) => {
      if (e.target === fullscreenGallery) {
        closeFullscreenGallery();
      }
    };
    fullscreenGallery.addEventListener('click', fullscreenGallery._fullscreenBackgroundClick);
  }
}

function removeFullscreenEventHandlers() {
  const fullscreenGallery = document.getElementById('fullscreenGallery');
  const fullscreenImg = document.getElementById('fullscreenImg');
  const fullscreenCloseBtn = document.getElementById('fullscreenCloseBtn');
  
  if (fullscreenCloseBtn) {
    fullscreenCloseBtn.removeEventListener('click', closeFullscreenGallery);
  }
  
  if (fullscreenImg) {
    fullscreenImg.removeEventListener('touchstart', fullscreenImg._fullscreenTouchStart);
    fullscreenImg.removeEventListener('touchend', fullscreenImg._fullscreenTouchEnd);
    fullscreenImg.removeEventListener('click', fullscreenImg._fullscreenClick);
  }
  
  if (fullscreenGallery) {
    fullscreenGallery.removeEventListener('click', fullscreenGallery._fullscreenBackgroundClick);
  }
}

function updateFullscreenImage() {
  const fullscreenImg = document.getElementById('fullscreenImg');
  if (fullscreenImg && current && current.images[curIndex]) {
    fullscreenImg.src = `img/${current.images[curIndex]}`;
  }
}
...
function updateMobileImage() {
  const mobileMainImg = document.getElementById('mobileMainImg');
  if (mobileMainImg && current && current.images[curIndex]) {
    mobileMainImg.src = `img/${current.images[curIndex]}`;
  }
}
