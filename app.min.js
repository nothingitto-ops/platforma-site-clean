...
function addCardEventListeners() {
  ...
  cards.forEach(card => {
    ...
    if (item) {
      // Обработчик клика с защитой от двойных кликов
      ...
      card.addEventListener('touchend', (e) => {
        // Предотвращаем конфликты с touch событиями
        e.preventDefault();
        console.log('Touch по карточке:', item.title);
        openModal(item, currentTab);
      }, { passive: false });
...
function initScrollHandler() {
  ...
  // Предотвращаем "рефреш" интерфейса после скролла
  // Добавляем небольшую задержку для стабилизации
  clearTimeout(window.scrollTimeout);
  window.scrollTimeout = setTimeout(() => {
    // Принудительно перерисовываем каталог для стабильности
    const catalog = document.getElementById('catalog');
    if (catalog && catalog.style.display === 'grid') {
      // Временно скрываем и показываем для перерисовки
      catalog.style.opacity = '0.99';
