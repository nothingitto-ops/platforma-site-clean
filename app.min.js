...
function loadHero() {
  ...
  if (hero) {
    ...
    hero.src = `img/${banner}?v=${new Date().getTime()}`;
...
function createCard(item) {
  ...
  return `
    <div class="card" data-id="${item.id}">
      <div class="card-img">
        <img src="img/${item.images[0]}?v=${new Date().getTime()}" alt="${item.title}" loading="lazy">
      </div>
      <h3>${item.title}</h3>
        ...
        if (imgBox && img && item.images.length > 1) {
          ...
          const applyIdx = (idx) => {
            if (idx !== currentHoverIndex && idx < item.images.length) {
              ...
              img.src = `img/${item.images[idx]}?v=${new Date().getTime()}`;
            }
          };
