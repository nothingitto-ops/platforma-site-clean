...
function openMobileModal(item, section) {
    ...
    if (current && current.images.length > 1) {
      ...
      mobileMainImage._touchEndHandler = (e) => {
        if (e.changedTouches.length === 1 && isSwiping) {
          ...
          // Упрощенные условия для максимальной скорости
          if (Math.abs(deltaX) > 10) {
            ...
            if (mobileMainImg) {
              mobileMainImg.src = `img/${current.images[curIndex]}`;
              // Обновляем счетчик асинхронно для скорости
  ...
  if (current && current.images.length > 1) {
    ...
    const preloadPromises = current.images.map(image => {
    ...
    // Не ждем завершения, просто запускаем предзагрузку
