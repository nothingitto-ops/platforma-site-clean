    ...
    if (current && current.images.length > 1) {
      ...
      mobileMainImage._touchMoveHandler = (e) => {
        if (e.touches.length === 1) {
          ...
          // Определяем, что это горизонтальный свайп - еще больше уменьшаем порог
          if (deltaX > 3 && deltaX > deltaY) {
            isSwiping = true;
          }
        }
      };
      ...
      mobileMainImage._touchEndHandler = (e) => {
        if (e.changedTouches.length === 1 && isSwiping) {
          ...
          // Проверяем условия для свайпа - максимально быстрый отклик
          if (Math.abs(deltaX) > 15 && deltaTime < 200 && deltaY < 50) {
            ...
            // Мгновенно обновляем изображение без проверок
            const mobileMainImg = document.getElementById('mobileMainImg');
            if (mobileMainImg) {
              ...
              mobileMainImg.style.display = 'none';
              mobileMainImg.offsetHeight; // Форсируем перерисовку
              mobileMainImg.style.display = '';
            }
          }
        }
      };
      ...
      mobileMainImage.addEventListener('touchstart', mobileMainImage._touchStartHandler, { passive: true, capture: true });
      mobileMainImage.addEventListener('touchmove', mobileMainImage._touchMoveHandler, { passive: true, capture: true });
      mobileMainImage.addEventListener('touchend', mobileMainImage._touchEndHandler, { passive: false, capture: true });
  }
    ...
    if (current && current.images.length > 1) {
      ...
      mobileZoomImg._swipeTouchStart = (e) => {
        if (e.touches.length === 1) {
          ...
          // Проверяем текущее состояние зума в начале касания
          const transform = window.getComputedStyle(mobileZoomImg).transform;
          const matrix = new DOMMatrix(transform);
          const currentScale = matrix.a;
          isZoomed = currentScale > 1.02; // Еще больше уменьшаем порог для более стабильного определения зума
          
          // Если в зуме - принудительно сбрасываем флаг свайпа и блокируем листание
          if (isZoomed) {
            isSwiping = false;
            // Дополнительно блокируем любые попытки листания
            e.preventDefault();
            e.stopPropagation();
          }
        }
      ...
      mobileZoomImg._swipeTouchMove = (e) => {
        if (e.touches.length === 1) {
          ...
          const matrix = new DOMMatrix(transform);
          const currentScale = matrix.a;
          const isCurrentlyZoomed = currentScale > 1.02; // Уменьшаем порог для более стабильного определения зума
          
          // Если в зуме - ПОЛНОСТЬЮ блокируем все движения
          if (isCurrentlyZoomed) {
            isSwiping = false; // Принудительно сбрасываем флаг свайпа
            // Блокируем ВСЕ движения, которые могут вызвать листание
            e.preventDefault();
            e.stopPropagation();
            return;
          }
          ...
          if (!isCurrentlyZoomed && deltaX > 10 && deltaX > deltaY) {
            isSwiping = true;
          }
        }
      };
      ...
      mobileZoomImg._gestureEnd = (e) => {
        ...
        // Если масштаб больше 1.02 - обновляем флаг зума
        if (currentScale > 1.02) {
          isZoomed = true;
        }
      ...
      mobileZoomImg._swipeTouchEnd = (e) => {
        ...
        // Если в зуме - ПОЛНОСТЬЮ блокируем листание изображений
        if (isCurrentlyZoomed) {
          isSwiping = false; // Принудительно сбрасываем флаг свайпа
          e.preventDefault(); // Дополнительно предотвращаем любые действия
          e.stopPropagation();
          e.stopImmediatePropagation(); // Останавливаем все обработчики
          return false; // Возвращаем false для полной блокировки
        }
      ...
      mobileZoomImg.addEventListener('touchstart', mobileZoomImg._swipeTouchStart, { passive: false });
      mobileZoomImg.addEventListener('touchmove', mobileZoomImg._swipeTouchMove, { passive: false });
      ...
      mobileZoomImg.addEventListener('gesturestart', (e) => {
        ...
        // Проверяем состояние зума и блокируем листание если нужно
        const transform = window.getComputedStyle(mobileZoomImg).transform;
        const matrix = new DOMMatrix(transform);
        const currentScale = matrix.a;
        if (currentScale > 1.02) {
      ...
      mobileZoomImg.addEventListener('gesturechange', (e) => {
        ...
        const matrix = new DOMMatrix(transform);
        const currentScale = matrix.a;
        isZoomed = currentScale > 1.02; // Уменьшаем порог для более стабильного определения зума
