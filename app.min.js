...
function addPanHandlers() {
  ...
  mobileMainImg._panMove = (e) => {
    ...
    // При зуме 2x максимальное смещение = размер контейнера минус размер изображения
    const containerWidth = mobileMainImage.offsetWidth;
    const containerHeight = mobileMainImage.offsetHeight;
    const imgWidth = mobileMainImg.offsetWidth;
    const imgHeight = mobileMainImg.offsetHeight;
    
    // Максимальное смещение = разность между размером увеличенного изображения и контейнера
    const maxX = Math.max(0, (imgWidth * 2 - containerWidth) / 2);
    const maxY = Math.max(0, (imgHeight * 2 - containerHeight) / 2);
