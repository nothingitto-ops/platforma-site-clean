...
function addCardEventListeners() {
  ...
  cards.forEach(card => {
    ...
    if (item) {
      // Обработчик клика - улучшенный для мобильных устройств
      ...
      card.removeEventListener('touchend', clickHandler);
      
      // Добавляем только обработчик клика (не touchstart!)
      ...
      card.addEventListener('touchend', (e) => {
        ...
        // Если это быстрый тап (менее 300мс) и небольшое движение (менее 10px) - считаем кликом
...
function openMobileModal(item, section) {
  ...
  // Предзагружаем изображения для быстрой работы
  ...
  if (mobileModalElement) {
    ...
    if (current && current.images.length > 1) {
      ...
      mobileMainImage._touchStartHandler = (e) => {
        // Если зум активен, не обрабатываем жесты для листания
        if (mobileMainImage.classList.contains('zoomed')) {
          return;
        }
...
function addMobileEventHandlers() {
  ...
  if (mobileZoomBtn) {
    ...
    mobileZoomBtn.removeEventListener('touchstart', mobileZoomBtn._touchHandler);
    
    // Создаем новый обработчик для iPhone
    mobileZoomBtn._touchHandler = (e) => {
      e.preventDefault();
      e.stopPropagation();
      e.stopImmediatePropagation();
      
      // Небольшая задержка для стабильности на iPhone
      setTimeout(() => {
        toggleMobileZoom();
      }, 50);
    };
    ...
    mobileZoomBtn.addEventListener('click', mobileZoomBtn._touchHandler, { passive: false });
    mobileZoomBtn.addEventListener('touchstart', mobileZoomBtn._touchHandler, { passive: false });
    
    // Дополнительно для iPhone
    mobileZoomBtn.addEventListener('touchend', (e) => {
      e.preventDefault();
      e.stopPropagation();
    }, { passive: false });
  }
...
function toggleMobileZoom() {
  ...
  if (!mobileMainImage || !mobileMainImg) return;
  ...
  if (isZoomed) {
    ...
    mobileMainImg.style.transform = 'scale(1.2) translate(0, 0)';
    mobileMainImg.style.webkitTransform = 'scale(1.2) translate(0, 0)';
    
    // Принудительно обновляем стили
    mobileMainImg.offsetHeight; // Trigger reflow
...
function addPanHandlers() {
  const mobileMainImg = document.getElementById('mobileMainImg');
  const mobileMainImageContainer = document.querySelector('.mobile-main-image');
  if (!mobileMainImg || !mobileMainImageContainer) return;
  ...
  // Инициализируем координаты с нуля при включении зума
  currentX = 0;
  currentY = 0;
  
  console.log('Pan handlers initialized with coordinates:', { currentX, currentY });
  ...
  mobileMainImg._panMove = (e) => {
    ...
    // Добавляем плавность с коэффициентом 0.8
    const smoothFactor = 0.8;
    const newX = currentX + deltaX * smoothFactor;
    const newY = currentY + deltaY * smoothFactor;
    
    // Блокируем все другие события при перемещении
    e.preventDefault();
    e.stopPropagation();
    e.stopImmediatePropagation();
    
    // Отладочная информация
    console.log('Panning:', { deltaX, deltaY, newX, newY, currentX, currentY });
    
    // Ограничиваем перемещение строго в пределах фото
    const containerWidth = mobileMainImageContainer.offsetWidth;
    const containerHeight = mobileMainImageContainer.offsetHeight;
    const imgWidth = mobileMainImg.naturalWidth || mobileMainImg.offsetWidth;
    const imgHeight = mobileMainImg.naturalHeight || mobileMainImg.offsetHeight;
    
    // При зуме 1.2x изображение увеличивается в 1.2 раза
    const zoomedImgWidth = imgWidth * 1.2;
    const zoomedImgHeight = imgHeight * 1.2;
    
    // Максимальное смещение = половина разности между размером увеличенного изображения и контейнера
    const maxX = Math.max(0, (zoomedImgWidth - containerWidth) / 2);
    const maxY = Math.max(0, (zoomedImgHeight - containerHeight) / 2);
    
    const clampedX = Math.max(-maxX, Math.min(maxX, newX));
    const clampedY = Math.max(-maxY, Math.min(maxY, newY));
    
    // Применяем transform с принудительным обновлением
    mobileMainImg.style.transform = `scale(1.2) translate(${clampedX}px, ${clampedY}px)`;
    
    // Принудительно обновляем стили
    mobileMainImg.style.webkitTransform = `scale(1.2) translate(${clampedX}px, ${clampedY}px)`;
    mobileMainImg.style.mozTransform = `scale(1.2) translate(${clampedX}px, ${clampedY}px)`;
    mobileMainImg.style.msTransform = `scale(1.2) translate(${clampedX}px, ${clampedY}px)`;
    
    // ПРИНУДИТЕЛЬНО ОБНОВЛЯЕМ DOM
    mobileMainImg.offsetHeight;
    mobileMainImg.offsetWidth;
    mobileMainImg.getBoundingClientRect();
    
    // Принудительно обновляем стили контейнера
    const mobileMainImageContainer = document.querySelector('.mobile-main-image.zoomed');
    if (mobileMainImageContainer) {
      mobileMainImageContainer.style.pointerEvents = 'auto';
      mobileMainImageContainer.style.touchAction = 'pan-x pan-y';
    }
    
    // Отладочная информация
    console.log('Applied transform:', `scale(1.2) translate(${clampedX}px, ${clampedY}px)`);
    console.log('Image position:', mobileMainImg.getBoundingClientRect());
  ...
  mobileMainImg._panEnd = () => {
    if (isPanning) {
      // Обновляем текущие координаты из последнего transform
      ...
      if (translateMatch) {
        ...
        console.log('Updated coordinates:', { currentX, currentY });
      } else {
        // Если не можем получить из transform, используем последние вычисленные значения
        console.log('Using computed coordinates:', { currentX, currentY });
      }
      isPanning = false;
    }
  };
