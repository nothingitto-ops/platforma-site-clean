...
function openMobileModal(item, section) {
  ...
  if (item.images.length > 1) {
    console.log(`Создаем точки: curIndex = ${curIndex}, images count = ${item.images.length}`);
...
function updateMobileDots() {
  ...
  dots.forEach((dot) => {
    ...
    console.log(`Точка ${dot.getAttribute('data-index')}: dataIndex = ${dataIndex}, curIndex = ${curIndex}, isActive = ${isActive}`);
  ...
  // Отладочная информация для проверки логики точек
  ...
  console.log(`updateMobileDots: curIndex = ${curIndex}, total dots = ${dots.length}, active dots = ${activeDots.length}, active indexes = [${activeIndexes.join(', ')}]`);
}
...
function updateZoomDots() {
  ...
  // Отладочная информация для проверки логики точек
  console.log(`updateZoomDots: curIndex = ${curIndex}, total dots = ${dots.length}, active dots = ${Array.from(dots).filter(d => d.classList.contains('active')).length}`);
}
