      ...
      if (e.changedTouches.length === 1) {
        ...
        if (Math.abs(deltaX) > 50 && deltaTime < 300 && deltaY < 30 && current && current.images.length > 1) {
          ...
          if (oldIndex !== curIndex) {
            ...
            console.log(`Путь к изображению: img/${current.images[curIndex]}`);
            const mobileMainImg = document.getElementById('mobileMainImg');
            if (mobileMainImg) {
              mobileMainImg.src = `img/${current.images[curIndex]}`;
              console.log(`Установлен src: ${mobileMainImg.src}`);
            }
          }
        }
        
        // Закрытие модалки при перетягивании с левого края экрана
        if (deltaX > 100 && deltaTime > 300 && deltaY < 50 && touchStartX < 50) {
          console.log('Закрытие модалки перетягиванием с левого края');
...
function updateMobileDots() {
  // Функция больше не нужна, так как точки убраны
  return;
}
...
function closeMobileModalWithAnimation() {
  // Убираем анимацию - закрываем сразу
  closeAllModals();
}
  ...
  if (mobileModal) {
    ...
    mobileModal.addEventListener('touchend', (e) => {
      if (e.changedTouches.length === 1) {
        ...
        if (deltaX > 100 && deltaTime > 300 && deltaY < 50 && touchStartX < 50) {
          console.log('Закрытие основной модалки жестом "назад"');
          closeMobileModalWithAnimation();
          return;
        }
      }
...
function addMobileZoomEventHandlers() {
  ...
  if (mobileZoomModal) {
    ...
    mobileZoomModal.addEventListener('touchstart', (e) => {
      if (e.touches.length === 1) {
        touchStartX = e.touches[0].clientX;
        touchStartY = e.touches[0].clientY;
        touchStartTime = Date.now();
    ...
    mobileZoomModal.addEventListener('touchend', (e) => {
      if (e.changedTouches.length === 1) {
        const touchEndX = e.changedTouches[0].clientX;
        const touchEndY = e.changedTouches[0].clientY;
        const touchEndTime = Date.now();
        ...
        // Жест "назад" в Safari: свайп справа налево с левого края экрана
        if (deltaX > 100 && deltaTime > 300 && deltaY < 50 && touchStartX < 50) {
          console.log('Закрытие зум-модалки жестом "назад"');
          closeMobileZoom();
          return;
        }
      }
  ...
  if (mobileZoomImg) {
    ...
    // Добавляем обработчик для сброса положения при слишком далеком отдалении
    mobileZoomImg.addEventListener('gestureend', (e) => {
      // Получаем текущий масштаб
      const transform = window.getComputedStyle(mobileZoomImg).transform;
      const matrix = new DOMMatrix(transform);
      currentScale = matrix.a; // a и d компоненты матрицы содержат масштаб
      ...
      if (e.changedTouches.length === 1 && current && current.images.length > 1) {
        ...
        if (Math.abs(deltaX) > 50 && deltaTime < 300 && deltaY < 30) {
          ...
          if (oldIndex !== curIndex) {
            ...
            if (mobileMainImg) {
              mobileMainImg.src = `img/${current.images[curIndex]}`;
              console.log(`Установлен src основной: ${mobileMainImg.src}`);
            }
            mobileZoomImg.src = `img/${current.images[curIndex]}`;
            console.log(`Установлен src зум: ${mobileZoomImg.src}`);
  ...
  // Добавляем обработчик на контейнер зум модалки для лучшего листания
  const mobileZoomContent = document.querySelector('.mobile-zoom-content');
  if (mobileZoomContent && current && current.images.length > 1) {
    let touchStartX = 0;
    let touchStartY = 0;
    let touchStartTime = 0;
    ...
    mobileZoomContent.addEventListener('touchend', (e) => {
      if (e.changedTouches.length === 1) {
        const touchEndX = e.changedTouches[0].clientX;
        const touchEndY = e.changedTouches[0].clientY;
        const touchEndTime = Date.now();
        ...
        if (Math.abs(deltaX) > 50 && deltaTime < 300 && deltaY < 30) {
          ...
          // Проверяем, что индекс действительно изменился
          if (oldIndex !== curIndex) {
            console.log(`Зум контейнер свайп: ${oldIndex} -> ${curIndex} (всего: ${current.images.length})`);
            console.log(`Путь к изображению: img/${current.images[curIndex]}`);
            const mobileMainImg = document.getElementById('mobileMainImg');
            const mobileZoomImg = document.getElementById('mobileZoomImg');
            if (mobileMainImg) {
