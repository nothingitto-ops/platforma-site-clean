  ...
  if (mobileZoomImg) {
    ...
    if (current && current.images.length > 1) {
      ...
      mobileZoomImg._swipeTouchStart = (e) => {
        // СРАЗУ проверяем состояние зума с более строгим порогом
        const currentScale = getCurrentScale(mobileZoomImg);
        
        // Альтернативный способ определения зума через размеры
        const naturalWidth = mobileZoomImg.naturalWidth;
        const displayWidth = mobileZoomImg.offsetWidth;
        const displayHeight = mobileZoomImg.offsetHeight;
        const naturalHeight = mobileZoomImg.naturalHeight;
        
        const scaleBySize = Math.max(displayWidth / naturalWidth, displayHeight / naturalHeight);
        
        // Используем максимальное значение из двух способов
        const finalScale = Math.max(currentScale, scaleBySize);
        isZoomed = finalScale > 1.01;
        
        // Отладочная информация
        console.log('TouchStart - Scale:', currentScale, 'ScaleBySize:', scaleBySize, 'FinalScale:', finalScale, 'isZoomed:', isZoomed);
        
        // Визуальный индикатор на экране
        showDebugInfo('TouchStart', finalScale, isZoomed);
        
        // Если в зуме - ПОЛНОСТЬЮ блокируем все
        if (isZoomed) {
          isSwiping = false;
          e.preventDefault();
          e.stopPropagation();
        ...
        // Только если НЕ в зуме - обрабатываем касание
        if (e.touches.length === 1) {
          touchStartX = e.touches[0].clientX;
          touchStartY = e.touches[0].clientY;
          touchStartTime = Date.now();
      ...
      mobileZoomImg._swipeTouchMove = (e) => {
        if (e.touches.length === 1) {
          // ВСЕГДА проверяем состояние зума при каждом движении с более строгим порогом
          const currentScale = getCurrentScale(mobileZoomImg);
          ...
          // Используем максимальное значение из двух способов
          const finalScale = Math.max(currentScale, scaleBySize);
          const isCurrentlyZoomed = finalScale > 1.01;
          
          // Отладочная информация
          console.log('TouchMove - Scale:', currentScale, 'ScaleBySize:', scaleBySize, 'FinalScale:', finalScale, 'isZoomed:', isCurrentlyZoomed);
          
          // Визуальный индикатор на экране
          showDebugInfo('TouchMove', finalScale, isCurrentlyZoomed);
          
          // Если в зуме - ПОЛНОСТЬЮ блокируем все движения
      ...
      mobileZoomImg._gestureEnd = (e) => {
        ...
        if (currentScale < 0.5) {
          ...
          mobileZoomImg.classList.remove('zoom-blocked'); // Убираем блокировку
        ...
        if (currentScale > 1.01) {
          ...
          mobileZoomImg.classList.add('zoom-blocked'); // Добавляем блокировку
        }
      ...
      mobileZoomImg._clickHandler = (e) => {
        ...
        const matrix = new DOMMatrix(transform);
        const currentScale = matrix.a;
        const isCurrentlyZoomed = currentScale > 1.01; // Уменьшаем порог для более стабильного определения зума
      ...
      mobileZoomImg._swipeTouchEnd = (e) => {
        ...
        // Отладочная информация
        console.log('TouchEnd - Scale:', currentScale, 'ScaleBySize:', scaleBySize, 'FinalScale:', finalScale, 'isZoomed:', isCurrentlyZoomed);
        
        // Визуальный индикатор на экране
        showDebugInfo('TouchEnd', finalScale, isCurrentlyZoomed);
      ...
      mobileZoomImg.addEventListener('gesturechange', (e) => {
        ...
        // Если в зуме - принудительно сбрасываем флаг свайпа и блокируем листание
        if (isZoomed) {
          isSwiping = false;
          // Дополнительно блокируем любые жесты листания
          e.preventDefault();
          e.stopPropagation();
          e.stopImmediatePropagation();
          
          // Добавляем CSS класс для блокировки
          mobileZoomImg.classList.add('zoom-blocked');
          
          // Принудительно отключаем обработчики свайпов в режиме зума
          mobileZoomImg.removeEventListener('touchstart', mobileZoomImg._swipeTouchStart, { passive: false, capture: true });
          mobileZoomImg.removeEventListener('touchmove', mobileZoomImg._swipeTouchMove, { passive: false, capture: true });
          mobileZoomImg.removeEventListener('touchend', mobileZoomImg._swipeTouchEnd, { passive: false, capture: true });
        } else {
          // Если не в зуме - убираем CSS класс блокировки
          mobileZoomImg.classList.remove('zoom-blocked');
          
          // Если не в зуме - добавляем обработчики обратно
          mobileZoomImg.addEventListener('touchstart', mobileZoomImg._swipeTouchStart, { passive: false, capture: true });
          mobileZoomImg.addEventListener('touchmove', mobileZoomImg._swipeTouchMove, { passive: false, capture: true });
          mobileZoomImg.addEventListener('touchend', mobileZoomImg._swipeTouchEnd, { passive: false, capture: true });
        }
      ...
      // Глобальная блокировка листания в зум режиме
      const globalZoomBlocker = (e) => {
        const mobileZoomImg = document.getElementById('mobileZoomImg');
        if (mobileZoomImg) {
          const transform = window.getComputedStyle(mobileZoomImg).transform;
          const matrix = new DOMMatrix(transform);
          const currentScale = matrix.a;
          if (currentScale > 1.01) {
            // Принудительно сбрасываем флаг свайпа
            isSwiping = false;
            isZoomed = true;
            e.preventDefault();
            e.stopPropagation();
      ...
      // Добавляем глобальные обработчики для блокировки листания
      document.addEventListener('touchmove', globalZoomBlocker, { passive: false, capture: true });
      document.addEventListener('touchend', globalZoomBlocker, { passive: false, capture: true });
    }
  }
