  ...
  if (mobileZoomImg) {
    ...
    if (current && current.images.length > 1) {
      ...
      mobileZoomImg._swipeTouchStart = (e) => {
        ...
        isZoomed = isImageZoomed(mobileZoomImg);
        
        // Дополнительная проверка через viewport scale
        const viewportScale = window.visualViewport ? window.visualViewport.scale : 1;
        let finalZoomed = isZoomed;
        if (viewportScale > 1.01) {
          finalZoomed = true;
        ...
        console.log('TouchStart - isZoomed:', finalZoomed, 'ViewportScale:', viewportScale);
        
        // Визуальный индикатор на экране
        showDebugInfo('TouchStart', finalZoomed ? 2 : 1, finalZoomed);
      ...
      mobileZoomImg._swipeTouchMove = (e) => {
        if (e.touches.length === 1) {
          ...
          const isCurrentlyZoomed = isImageZoomed(mobileZoomImg);
          
          // Дополнительная проверка через viewport scale
          const viewportScale = window.visualViewport ? window.visualViewport.scale : 1;
          let finalZoomed = isCurrentlyZoomed;
          if (viewportScale > 1.01) {
            finalZoomed = true;
            console.log('Forced zoom detection via viewport scale:', viewportScale);
          }
          ...
          // Отладочная информация
          console.log('TouchMove - isZoomed:', finalZoomed, 'ViewportScale:', viewportScale);
          
          // Визуальный индикатор на экране
          showDebugInfo('TouchMove', finalZoomed ? 2 : 1, finalZoomed);
          
          // Если в зуме - блокируем только горизонтальные свайпы для листания
          if (finalZoomed) {
            isSwiping = false;
            isZoomed = true; // Обновляем глобальный флаг
            
            // Разрешаем вертикальные движения для панорамирования
            const currentX = e.touches[0].clientX;
            const currentY = e.touches[0].clientY;
            const deltaX = Math.abs(currentX - touchStartX);
            const deltaY = Math.abs(currentY - touchStartY);
            
            // Блокируем только горизонтальные свайпы (листание)
            ...
            // Разрешаем вертикальные движения (панорамирование)
            return;
          }
