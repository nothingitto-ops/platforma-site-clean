...
function openMobileModal(item, section) {
  ...
  if (mobileMainImage) {
    ...
    let isProcessingSwipe = false; // Защита от множественных срабатываний
    ...
    mobileMainImage.addEventListener('touchstart', (e) => {
      if (e.touches.length === 1) {
        ...
        isProcessingSwipe = false; // Сбрасываем флаг при новом касании
      }
    }, { passive: true });
    ...
    mobileMainImage.addEventListener('touchmove', (e) => {
      if (e.touches.length === 1) {
        ...
        // Предотвращаем только вертикальные свайпы, разрешаем горизонтальные
      ...
      if (e.changedTouches.length === 1) {
        ...
        if (Math.abs(deltaX) > 50 && deltaTime < 300 && deltaY < 30 && current && current.images.length > 1) {
          ...
          if (oldIndex !== curIndex) {
            console.log(`Свайп: ${oldIndex} -> ${curIndex} (всего: ${current.images.length})`);
            const mobileMainImg = document.getElementById('mobileMainImg');
            if (mobileMainImg) {
              mobileMainImg.src = `img/${current.images[curIndex]}`;
            }
        ...
        // Если это медленный свайп влево для закрытия модалки
        if (deltaX > 100 && deltaTime > 500 && deltaY < 50) {
          console.log('Закрытие модалки свайпом влево');
          closeMobileModal();
        }
      }
    }, { passive: false });
  ...
  // Создаем точки для навигации по изображениям - НОВАЯ ВЕРСИЯ
  mobileDots.innerHTML = '';
  if (item.images.length > 1) {
    console.log(`Создаем точки заново: curIndex = ${curIndex}, images count = ${item.images.length}`);
    item.images.forEach((imagePath, idx) => {
      const dot = document.createElement('div'); // Используем div вместо button
...
function updateMobileDots() {
  ...
  console.log(`updateMobileDots: curIndex = ${curIndex}, total dots = ${dots.length}`);
  
  dots.forEach((dot) => {
    const dataIndex = parseInt(dot.getAttribute('data-index'));
    const isActive = dataIndex === curIndex;
    
    console.log(`Точка ${dataIndex}: isActive = ${isActive}`);
  ...
  const activeDots = Array.from(dots).filter(d => d.classList.contains('active'));
  const activeIndexes = activeDots.map(d => d.getAttribute('data-index'));
  console.log(`Результат: active dots = ${activeDots.length}, active indexes = [${activeIndexes.join(', ')}]`);
}
...
function openMobileZoom() {
  ...
  // Создаем точки для зум модалки - НОВАЯ ВЕРСИЯ
  ...
  if (zoomDotsContainer && current && current.images.length > 1) {
    ...
    console.log(`Создаем зум-точки: curIndex = ${curIndex}, images count = ${current.images.length}`);
    
    current.images.forEach((imagePath, idx) => {
      const dot = document.createElement('div'); // Используем div вместо button
      dot.className = 'mobile-zoom-dot';
      dot.setAttribute('data-index', idx);
  ...
  if (mobileZoomContent) {
    // Убираем обработчик кликов для листания
...
function updateZoomDots() {
  ...
  console.log(`updateZoomDots: curIndex = ${curIndex}, total dots = ${dots.length}`);
  
  dots.forEach((dot) => {
    const dataIndex = parseInt(dot.getAttribute('data-index'));
    const isActive = dataIndex === curIndex;
    
    console.log(`Зум-точка ${dataIndex}: isActive = ${isActive}`);
  ...
  const activeDots = Array.from(dots).filter(d => d.classList.contains('active'));
  const activeIndexes = activeDots.map(d => d.getAttribute('data-index'));
  console.log(`Зум-результат: active dots = ${activeDots.length}, active indexes = [${activeIndexes.join(', ')}]`);
}
  ...
  if (mobileZoomImg) {
    ...
    mobileZoomImg.addEventListener('touchstart', (e) => {
      if (e.touches.length === 1) {
        ...
        isZooming = true;
        return; // Выходим, чтобы не мешать зуму
      }
    }, { passive: true });
    ...
    mobileZoomImg.addEventListener('touchmove', (e) => {
      ...
      // Если один палец - разрешаем все движения для зума
      if (e.touches.length === 1) {
        // Не блокируем движения - разрешаем зум
        return;
      }
    }, { passive: true });
    
    // Добавляем клик для выхода из зума
    mobileZoomImg.addEventListener('click', (e) => {
      e.preventDefault();
      e.stopPropagation();
      closeMobileZoom();
    });
    ...
    mobileZoomImg.addEventListener('touchend', (e) => {
      if (isZooming) {
        // Если был зум - не обрабатываем свайпы
        return;
      }
      ...
      if (e.changedTouches.length === 1 && current && current.images.length > 1) {
        ...
        // Если это медленный свайп влево для закрытия зум-модалки
        if (deltaX > 100 && deltaTime > 500 && deltaY < 50) {
          console.log('Закрытие зум-модалки свайпом влево');
          closeMobileZoom();
          return;
        }
        ...
        if (Math.abs(deltaX) > 50 && deltaTime < 300 && deltaY < 30) {
          ...
          // Проверяем, что индекс действительно изменился
          if (oldIndex !== curIndex) {
            console.log(`Зум свайп: ${oldIndex} -> ${curIndex} (всего: ${current.images.length})`);
            const mobileMainImg = document.getElementById('mobileMainImg');
            if (mobileMainImg) {
              mobileMainImg.src = `img/${current.images[curIndex]}`;
            }
