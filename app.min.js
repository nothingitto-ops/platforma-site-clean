    ...
    if (current && current.images.length > 1) {
      ...
      mobileMainImage._touchMoveHandler = (e) => {
        if (e.touches.length === 1) {
          ...
          // Более мягкие условия для Android
      ...
      // Добавляем обработчики с правильными настройками для Android
...
function addMobileZoomEventHandlers() {
  ...
  if (mobileZoomModal) {
    ...
    // Блокируем только горизонтальные свайпы в режиме зума для предотвращения листания
    mobileZoomModal.addEventListener('touchmove', (e) => {
      ...
      if (mobileZoomImg) {
        ...
        // Если в зуме - блокируем только горизонтальные движения для предотвращения листания
        if (currentScale > 1.02) {
          const currentX = e.touches[0].clientX;
          const currentY = e.touches[0].clientY;
          const deltaX = Math.abs(currentX - touchStartX);
  ...
  if (mobileZoomImg) {
    ...
    if (current && current.images.length > 1) {
      ...
      mobileZoomImg._swipeTouchStart = (e) => {
        ...
        // Устанавливаем координаты начала касания
        if (e.touches.length === 1) {
          touchStartX = e.touches[0].clientX;
          touchStartY = e.touches[0].clientY;
          touchStartTime = Date.now();
      ...
      mobileZoomImg._swipeTouchMove = (e) => {
        if (e.touches.length === 1) {
          ...
          // Если НЕ в зуме - обрабатываем свайпы для листания
          const currentX = e.touches[0].clientX;
          const currentY = e.touches[0].clientY;
          const deltaX = Math.abs(currentX - touchStartX);
          const deltaY = Math.abs(currentY - touchStartY);
          
          // Блокируем вертикальные движения только если не в зуме
          if (deltaY > 5) {
            e.preventDefault();
            e.stopPropagation();
            return;
          ...
          // Определяем горизонтальный свайп для листания
      ...
      mobileZoomImg._swipeTouchEnd = (e) => {
        ...
        // Если в зуме - БЛОКИРУЕМ листание, разрешаем только панорамирование
        if (isCurrentlyZoomed) {
          isSwiping = false;
          isZoomed = true;
      ...
      // Добавляем обработчики с правильными настройками для панорамирования
      ...
      // Блокируем wheel события в режиме зума для предотвращения случайного листания
