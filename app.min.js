  ...
  if (mobileMainImage) {
    ...
    mobileMainImage.addEventListener('click', (e) => {
      ...
      if (e.target.closest('.mobile-close-btn')) {
        console.log('Клик по кнопке закрытия, не открываем зум');
        return; // Не открываем зум если клик по крестику
      }
      console.log('Открываем зум...');
      openMobileZoom(e);
    });
  }
...
function openMobileZoom() {
  console.log('Функция openMobileZoom вызвана');
  const mobileZoomModal = document.getElementById('mobileZoomModal');
  const mobileZoomImg = document.getElementById('mobileZoomImg');
  const mobileMainImg = document.getElementById('mobileMainImg');
  
  console.log('Элементы:', { mobileZoomModal, mobileZoomImg, mobileMainImg });
  
  if (!mobileZoomModal || !mobileZoomImg || !mobileMainImg) {
    console.error('Не найдены элементы зум модалки');
    return;
  }
...
function closeMobileZoom() {
  console.log('Функция closeMobileZoom вызвана');
  const mobileZoomModal = document.getElementById('mobileZoomModal');
  if (mobileZoomModal) {
    console.log('Закрываем зум модалку');
    mobileZoomModal.classList.remove('open');
    // Возвращаемся в обычную модалку, не закрываем все
    document.body.style.overflow = 'hidden';
  } else {
    console.error('Элемент mobileZoomModal не найден');
  }
}
